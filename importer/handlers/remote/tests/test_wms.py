...
from .api.exception import ImportException
from django.contrib.auth import get_user_model
from .handlers.remote.serializers.wms import RemoteWMSSerializer
from .handlers.remote.wms import RemoteWMSResourceHandler
...
class TestRemoteWMSResourceHandler(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.valid_url = "https://development.demo.geonode.org/geoserver/ows?service=WMS&version=1.3.0&request=GetCapabilities"
    ...
    def test_import_resource_should_work(self, patch_upload):
        ...
        try:
            ...
            self.handler.import_resource(
                files=self.valid_payload_with_parse_false, execution_id=str(exec_id)
            )
    ...
    def test_create_geonode_resource(self):
        ...
        self.assertEqual(resource.subtype, "remote")
        self.assertTrue(
            ResourceBase.objects.filter(alternate="layeralternate").exists()
        )
